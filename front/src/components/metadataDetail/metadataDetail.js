...
class MetadataDetail extends Component {
        ...
        {!this.state.isFetching && !this.state.isError && (
          <div className="card tempPaddingTop">
            ...
            <div className="card-body">
              ...
              <div>
                {this.state.metadata.keywords.map((keyword, i) => {
                  return (
                    <a href="#" className="btn btn-secondary" key={keyword + i}>
                      {keyword}
                    </a>
                  );
                })}
              </div>
              {this.state.metadata.local_id && (
                <div className="card-text">
                  Id Local :<small className="text-muted">{this.state.metadata.local_id}</small>
                </div>
              )}
              <div className="card-text">
                DOI : <small className="text-muted">{this.state.metadata.doi}</small>
              </div>
              <div className="card-text">
                Producteur :
                <small className="text-muted">
                  {this.state.metadata.producer.organization_name}
                </small>
              </div>
                  ...
                  return (
                    <div className="col-4" key={contact.contact_id + i}>
                      <div className="card">
                        ...
                        <div className="card-text">
                          Mail : <small className="text-muted">{contact.email}</small>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="card-text">
                ...
                {this.isFlag &&
                  this.state.metadata.resource_languages.map((flag, i) => {
                    return <ReactCountryFlag key={lang} countryCode={flag} svg title={flag} />;
                  })}
                {!this.isFlag &&
                  this.state.metadata.resource_languages.map((lang, i) => {
                    return <span key={lang}>{ISO6391.getNativeName(lang)} </span>;
                  })}
              </div>
              ...
              <div className="card-text">
                geography : <small className="text-muted">TODO</small>
              </div>
              <div className="row">
                <div className="col-4">
                  <div className="card">
                    ...
                    {this.state.metadata.dataset_dates.validated && (
                      <div className="card-text">
                        Validé :
                        <small className="text-muted">
                          <Moment format="DD/MM/YYYY HH:mm:ss">
                    ...
                    {this.state.metadata.dataset_dates.updated && (
                      <div className="card-text">
                        Mis a jour :
                        <small className="text-muted">
                          <Moment format="DD/MM/YYYY HH:mm:ss">
              ...
              <div className="card-text">
                available_formats :
                <small className="text-muted">
                  {this.state.metadata.available_formats.media_type}
                </small>
              </div>
            </div>
          </div>
  ...
  fetchMetadata() {
    ...
    axios
      .get(`${process.env.PUBLIC_URL}/api/v1/resources/` + this.id)
      .then((response) => {
        this.setState({ metadata: response.data, isFetching: false, isError: false });
      })
      .catch((e) => {
        console.log(e);
