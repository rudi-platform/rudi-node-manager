...
export class RudiForm {
  constructor(language = 'fr') {
    ...
    this.customForm = document.getElementById('custom_form')
    if (!this.customForm) console.error('Form not yet loaded')
  ...
  ok = (here, ...args) => (this.isDev ? console.debug(`T OK [${here}]`, ...args) : null)
  ko = (here, ...args) => (this.isDev ? console.error(`ERR! [${here}]`, ...args) : null)

  importJSON = async (url, error_msg) => {
    try {
      return await JsonHttpRequest.get(url).send()
    } catch (err) {
  ...
  getLocal = async (...urlBits) => {
    try {
      return await JsonHttpRequest.get(this.getUrlLocal(...urlBits)).send()
    } catch {
      console.error(`Could not retrieve local address ${this.getUrlLocal(...urlBits)}`)
      return this.fail('reach_local')
    }
  }

  // Retrieve the authentification cookie
  _initPmHeaders = () => {
    const here = '_initPmHeaders'
    this.ok(here)
    if (this.isDev) {
      try {
        this._pmHeaders = {
          Authorization: `Bearer ${getCookie('consoleToken')}`,
          Accept: 'text/html',
        }
      } catch (e) {
        console.error(`Can't retrieve cookies:`, e)
        return this.fail('auth')
      }
    } else this._pmHeaders = { Accept: 'text/html' }
    return this._pmHeaders
  }

  _pmHeaders = null
  get pmHeaders() {
    if (!this._pmHeaders) this._initPmHeaders()
    return this._pmHeaders
  }
   ...
   *                    only mode
   * @param {String} objType RUDI type
   */
  async getEditModeAndFillData(objType) {
  ...
  devPaste() {
    try {
      const pastedJson = document.getElementById('dev_paste')
      pastedJson.focus()
      pastedJson.addEventListener('keydown', (element) => {
        if (element.key == 'Enter' && !element.shiftKey) {
          let val
          try {
            val = JSON.parse(pastedJson.value)
          } catch (e) {
            console.error(e)
          }
          ...
          this.setValue(val)
          if (element.ctrlKey) this.showResultOverlay()
        }
      })
    } catch (e) {
